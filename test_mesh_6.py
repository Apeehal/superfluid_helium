...
# Close the radial ends at the central axis and the outer boundary
for z in range(num_z):
    for theta in range(num_theta):
        ...
        p1 = idx(num_r_liquid, theta, z)
        p2 = idx(num_r_liquid, (theta + 1) % num_theta, z)
        p3 = idx(num_r_liquid, theta, z + 1)
        p4 = idx(num_r_liquid, (theta + 1) % num_theta, z + 1)
        faces.append([4, p1, p2, p4, p3])  # Quadrilateral for the outer section

# Add quadrilateral faces between radial divisions to close off surfaces
for z in range(num_z):
    for r in range(num_r_liquid):
        for theta in range(num_theta):
            # Get the indices of the four points forming a radial quad
            p1 = idx(r, theta, z)
            p2 = idx(r + 1, theta, z)
            p3 = idx(r + 1, theta, z + 1)
            p4 = idx(r, theta, z + 1)

            # Add the face connecting adjacent radial sections for each theta
...
# Perform a section cut along the x-z plane (y=0)
plane = pv.Plane(center=(0, 0, length / 2), direction=(0, 1, 0))

# Slice the mesh along the specified plane
sliced_mesh = mesh.slice_implicit(plane)

# Plot the sliced mesh (section view)
plotter = pv.Plotter()
plotter.add_mesh(sliced_mesh, color="lightblue", show_edges=True)
plotter.show()
